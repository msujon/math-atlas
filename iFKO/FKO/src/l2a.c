...
/*
 * Register alias for all vector types (for different vector-width)
 * NOTE: may be, we should move it to fko_arch when it gets finalized 
 */
#ifdef X86
   #if defined(AVX512)
      #ifdef X86_64
      ...
      #define archvs16regs archviregs  
      #define archvs8regs archymmregs  
      #define archvs4regs archxmmregs  
      #define archvi8regs archviregs  
      #define archvi4regs archymmregs  
      #define archvi2regs archxmmregs  
   #elif defined(AVX) 
      #ifdef X86_64
         extern char *archxmmregs[NFR];
      #else
      ...
      #define archvd2regs archxmmregs  
      #define archvs8regs archviregs  
      #define archvs4regs archxmmregs  
      #define archvi4regs archviregs  
      #define archvi2regs archxmmregs  
   #else /* SSE*/
      #define archvf4regs archvfregs  
      #define archvd2regs archvdregs  
      #define archvs4regs archviregs  
      #define archvi2regs archviregs  
   #endif
#endif
 ...
 */
      ...
      case UMUL:
            ...
            #endif
/*
 * NOTE: we use signed mul for umul on the x86, so we can use the
 ...
 *    But we can use it for YMM/XMM even in AVX512 
 */
      case CVTBFI:
         #ifdef AVX
 ...
 */
         if (op2 > 0)
	 {
	    assert(IS_GLOB(STflag[op2-1]));
	    #ifdef X86
               #if defined(AVX512) || defined(AVX) /*need to check the usage!... using vmovsd */
	          ap->next = PrintAssln("\tvmovsd\t%s,%s\n", STname[op2-1],
	                                archxmmregs[-DREGBEG-op1]);
               #else
	          ap->next = PrintAssln("\tmovlpd\t%s,%s\n", STname[op2-1],
 ...
 *
 *   FIXME: AVX512??? possible to use simplier???
 */
      case FCMOV1:
            #if defined(AVX) 
               assert( (op1 < 0) && (op3 < 0));
...
#ifdef X86
/*
 *    NOTE: we use these scalar CMP in AVX-512.. just use XMM regs
 *    We are not using K-regs variants (EVEX) for AVX-512 
 */
      case FCMPWEQ:
         #if defined(AVX) || defined(AVX512)
            if (op3 < 0)
 ...
 */
         ...
         break;
/*
 * FKO supports vectorization for double precision only on X86
 */
   #ifdef X86
/*       
 ...
 */
   ...
   #endif
/*
 * FKO supports Vectorization Only on x86 for now 
 */
      #if 0
      case VFLD:
         #if defined(AVX512) || defined(AVX)
      ...
      case VFLD_W8:
            ap->next = PrintAssln("\tvmovaps\t%s, %s\n", 
                                  GetDeref(op2),
                                  archvf8regs[-VFREGBEG-op1]);
         break;
   #endif
      case VFLD_W4:
            ap->next = PrintAssln("\tmovaps\t%s, %s\n", 
                                  GetDeref(op2),
                                  archvf4regs[-VFREGBEG-op1]);
         break;
      #if 0
      case VFLDU:
         #if defined(AVX512) || defined(AVX)
      ...
      case VFLDS_W4:
         ...
         break;
      #if 0
      case VFLDH_W4:  /* NOTE: dest is also source */
         #if defined(AVX512) || defined(AVX)
            ap->next = PrintAssln("ERROR:\t%d %d %d %d\n", 
                                  ip->inst[0], op1, op2, op3);
         ...
         break;
      #endif
      /* VFLDL, VFLDH only supported with SSE inst */         
      case VFLDL_W4:  /* NOTE: dest is also source */
 ...
 */
      ...
      case VFSTNT_W8:
            ap->next = PrintAssln("\tvmovntps\t%s, %s\n", 
                                  archvf8regs[-VFREGBEG-op2],
                                  GetDeref(op1));
         break;
      #endif
      case VFSTNT_W4:
            ap->next = PrintAssln("\tmovntps\t%s, %s\n", 
                                  archvf4regs[-VFREGBEG-op2],
                                  GetDeref(op1));
         break;
      #if 0
      case VFST:
 ...
 */
      ...
      case VFMUL_W4:
         ...
         #endif
         if ( op3 > 0)
            ap->next = PrintAssln("\tmulps\t%s, %s, %s\n", 
                                  GetDeref(op3),
                                  archvf4regs[-VFREGBEG-op2],
                                  archvf4regs[-VFREGBEG-op1]);
         else
      ...
      case VFMAC_W8: 
	       ...
	       fko_error(__LINE__, "VFMAC Not found in this x86 arch!");
            #endif
         break;
      case VFMAC_W4:  /* not supported on SSE */
            #if IFKO_DEBUG_LEVEL >= 1
               assert( op1 < 0 && op2 < 0 && op3 != 0);
            #endif
            #if defined(FMA3)
      ...
      case VFSUB_W4:
         ...
         if (op3 > 0)
            ...
            ap->next = PrintAssln("\tsubps\t%s,%s,%s\n", 
                                  archvf4regs[-VFREGBEG-op3], 
                                  archvf4regs[-VFREGBEG-op1]);
         break;
      #if 0
	 ...
	 assert(DTabs);
      ...
      case VFMAX_W4:
         ...
         if (op3 > 0)
            ...
            ap->next = PrintAssln("\tmaxps\t%s,%s,%s\n", 
                                  archvf4regs[-VFREGBEG-op3], 
                                  archvf4regs[-VFREGBEG-op1]);
         break;
      #if 0
      ...
      case VFMIN_W4:
         ...
         if (op3 > 0)
            ...
            ap->next = PrintAssln("\tminps\t%s,%s,%s\n", 
                                  archvf4regs[-VFREGBEG-op3], 
                                  archvf4regs[-VFREGBEG-op1]);
         break;
      #if 0
 ...
 *    NOTE: no support for VFHADD_W16
 */
      #if 0
      case VFHADD:
         #ifdef AVX
 ...
 *    NOTE: vmovmskps not supported for ZMM regs 
 */
      #if 0
      case VFSBTI:
         #ifdef AVX
 ...
 *    NOTE: only supported in AVX and SSE 
 */
      case VDSBTI_W4:
            ap->next = PrintAssln("\tvmovmskpd\t%s,%s\n",
                                   archvd4regs[-VDREGBEG-op2],
 ...
 */
      ...
      case VFCMOV1_W8:
         ...
         break;
      #endif
      /* NO implementation VFCMOV1_W4 ... xmm regs  */
      #if 0
      case VFCMOV2:
         #if defined(AVX512) 
            assert( (op1 < 0) && (op3 < 0) && (op2 < 0));
 ...
 */
      ...
      #endif
/*
 *    NOTE: Not implemented for VFSHUF_W16 
 */
      #if defined(AVX512) || defined(AVX)
      case VFSHUF_W8:
         cp = imap2cmap(SToff[op3-1].i); /* return hex char */ 
...
#ifdef X86
/*
 *    NOTE: for AVX512, we need to use k-regs 
 */
      #if 0
      case CVTMASKFI:
         #ifdef AVX
 ...
 * FIXME: what happens in X8632... should we support both 32 and 64 bit int? 
 */        
 ...
 */
   ...
   case VSMOV_W16:
         ...
         break;
   case VIMOV_W8:
            ap->next = PrintAssln("\tvmovdqa64\t%s, %s\n", 
   ...
   case VSMOV_W8:
            ap->next = PrintAssln("\tvmovdqa\t%s, %s\n", 
                                  archvs8regs[-VIREGBEG-op2],
                                  archvs8regs[-VIREGBEG-op1]);
         break;
   case VIMOV_W4:
            ap->next = PrintAssln("\tvmovdqa\t%s, %s\n", 
                                  archvi4regs[-VIREGBEG-op2],
                                  archvi4regs[-VIREGBEG-op1]);
         break;
   #endif
   case VSMOV_W4:
            ap->next = PrintAssln("\tmovdqa\t%s, %s\n", 
                                  archvs4regs[-VIREGBEG-op2],
                                  archvs4regs[-VIREGBEG-op1]);
         break;
   case VIMOV_W2:
            ap->next = PrintAssln("\tmovdqa\t%s, %s\n", 
                                  archvi2regs[-VIREGBEG-op2],
                                  archvi2regs[-VIREGBEG-op1]);
         break;
   #if 0
...
#ifdef X86_64
   #if defined(AVX512) || defined(AVX)
   case VSMOVS_W8: /* 32 bit int */
   #ifdef AVX512
   case VSMOVS_W16: /* 32 bit int */
   #endif
         op1 = -op1;
         op2 = -op2;
         op3 = op3 ? -op3: op3;
 ...
 */
   ...
   #endif
   #if defined(AVX512) || defined(AVX)
   case VSLDS_W8: /* 32 bit int */
      #ifdef AVX512
   case VSLDS_W16: /* 32 bit int */
      #endif
         assert(op1 < 0 && op2 >= 0);
         ap->next = PrintAssln("\tvmovd\t%s, %s\n", GetDeref(op2),
                               archxmmregs[-VIREGBEG-op1]); 
   ...
   #endif
   #if defined(AVX512) || defined(AVX)
   case VILDS_W4: /* 32 bit int */
      #ifdef AVX512
   case VILDS_W8: /* 32 bit int */
      #endif
         assert(op1 < 0 && op2 >= 0);
         #ifdef X86_64 
            ap->next = PrintAssln("\tvmovq\t%s, %s\n", GetDeref(op2),
   ...
   case VSST_W16:
         assert(op1 < 0 && op2 >= 0);
         ap->next = PrintAssln("\tvmovdqa32\t%s, %s\n", 
                               archvs16regs[-VIREGBEG-op2], GetDeref(op1) );
         break;
   case VIST_W8:
         assert(op1 < 0 && op2 >= 0);
         ap->next = PrintAssln("\tvmovdqa64\t%s, %s\n", 
                               archvi8regs[-VIREGBEG-op2], GetDeref(op1) );
         break;
   #endif
   #if defined(AVX512) || defined(AVX)
   case VSST_W8:
         assert(op1 < 0 && op2 >= 0);
         ap->next = PrintAssln("\tvmovdqa\t%s, %s\n", 
                               archvs8regs[-VIREGBEG-op2], GetDeref(op1) );
         break;
   case VIST_W4:
         assert(op1 < 0 && op2 >= 0);
         ap->next = PrintAssln("\tvmovdqa\t%s, %s\n", 
                               archvi4regs[-VIREGBEG-op2], GetDeref(op1) );
         break;
   #endif
   case VSST_W4:
         assert(op1 < 0 && op2 >= 0);
         ap->next = PrintAssln("\tmovdqa\t%s, %s\n", 
                               archvs4regs[-VIREGBEG-op2], GetDeref(op1) );
         break;
   case VIST_W2:
         assert(op1 < 0 && op2 >= 0);
         ap->next = PrintAssln("\tmovdqa\t%s, %s\n", 
                               archvi2regs[-VIREGBEG-op2], GetDeref(op1) );
         break;
   #if 0
   case VSTU:
         assert(op2 < 0 && op1 >= 0);
   ...
   #endif
   #if defined(AVX512) || defined(AVX)
   case VSSTS_W8: /* 32 bit int */
      #ifdef AVX512
   case VSSTS_W16: /* 32 bit int */
      #endif
         assert(op1 < 0 && op2 >= 0);
         ap->next = PrintAssln("\tvmovd\t%s, %s\n", 
                  archxmmregs[-VIREGBEG-op2], GetDeref(op1)); 
   ...
   #endif
   #if defined(AVX512) || defined(AVX)
   case VISTS_W4: /* 32 bit int */
      #ifdef AVX512
   case VISTS_W8: /* 32 bit int */
      #endif
         assert(op1 < 0 && op2 >= 0);
         #ifdef X86_64 
            ap->next = PrintAssln("\tvmovq\t%s, %s\n", 
 ...
 *       AVX version only supports on 128 bit (XMM regs)
 */
   ...
   case VSADD_W16:
         if (op3 > 0)
            ...
            ap->next = PrintAssln("\tvpaddd\t%s,%s,%s\n", 
                                  archvs16regs[-VIREGBEG-op3], 
                                  archvs16regs[-VIREGBEG-op2], 
                                  archvs16regs[-VIREGBEG-op1]);
         break;
   #endif
   #if defined(AVX512) || defined(AVX2)  /* AVX not supported on YMM */
   case VSADD_W8:
         if (op3 > 0)
            ap->next = PrintAssln("\tvpaddd\t%s,%s,%s\n", 
                                  GetDeref(op3),
   ...
   case VSADD_W4:
            assert(op1 == op2);
         if (op3 > 0)
            ap->next = PrintAssln("\tpaddd\t%s,%s,%s\n", 
                                  GetDeref(op3),
                                  archvs4regs[-VIREGBEG-op1]);
         else
            ap->next = PrintAssln("\tpaddd\t%s,%s,%s\n", 
                                  archvs4regs[-VIREGBEG-op3], 
                                  archvs4regs[-VIREGBEG-op1]);
         break;
#endif
   #if 0
 ...
 */
   ...
   case VIADD_W2:
            assert(op1 == op2);
         if (op3 > 0)
            ap->next = PrintAssln("\tpaddq\t%s,%s,%s\n", 
                                  GetDeref(op3),
                                  archvi2regs[-VIREGBEG-op1]);
         else
            ap->next = PrintAssln("\tpaddq\t%s,%s,%s\n", 
                                  archvi2regs[-VIREGBEG-op3], 
                                  archvi2regs[-VIREGBEG-op1]);
         break;
#ifdef X86_64
   #if 0
 ...
 */
   ...
   case VSSUB_W4:
            assert(op1 == op2);
         if (op3 > 0)
            ap->next = PrintAssln("\tpsubd\t%s,%s,%s\n", 
                                  GetDeref(op3),
                                  archvs4regs[-VIREGBEG-op1]);
         else
            ap->next = PrintAssln("\tpsubd\t%s,%s,%s\n", 
 ...
 */
   ...
   case VISUB_W2:
            assert(op1 == op2);
         if (op3 > 0)
            ap->next = PrintAssln("\tpsubq\t%s,%s,%s\n", 
                                  GetDeref(op3),
                                  archvi2regs[-VIREGBEG-op1]);
         else
            ap->next = PrintAssln("\tpsubq\t%s,%s,%s\n", 
 ...
 */
   ...
   #endif
/*
 * NOTE: we support MAX for signed int  
 */
   #ifdef AVX512
   case VSMAX_W16:
         if (op3 > 0)
   ...
   case VSMAX_W4:
            assert(op1 == op2);
         if (op3 > 0)
            ap->next = PrintAssln("\tpmaxsd\t%s,%s,%s\n", 
                                  GetDeref(op3),
                                  archvs4regs[-VIREGBEG-op1]);
         else
            ap->next = PrintAssln("\tpmaxsd\t%s,%s,%s\n", 
   ...
   case VIMAX_W8:
         if (op3 > 0)
            ap->next = PrintAssln("\tvpmaxsq\t%s,%s,%s\n", 
                                  GetDeref(op3),
                                  archvi8regs[-VIREGBEG-op2], 
                                  archvi8regs[-VIREGBEG-op1]);
         else
   ...
   #endif
   case VIMAX_W4:
            fko_error(__LINE__, "64 bit MAX not supported on YMM at all!");
         break;
   case VIMAX_W2:
            assert(op1 == op2);
         if (op3 > 0)
            ap->next = PrintAssln("\tpmaxsq\t%s,%s,%s\n", 
                                  GetDeref(op3),
                                  archvi2regs[-VIREGBEG-op1]);
         else
            ap->next = PrintAssln("\tpmaxsq\t%s,%s,%s\n", 
         ...
         break;
/*
 * NOTE: AVX & AVX2 only support 64bit max for XMM regs, not YMM regs
 */
#ifdef X86_64
   #if 0
   case VSMIN:
 ...
 */
   ...
   case VSMIN_W4:
            assert(op1 == op2);
         if (op3 > 0)
            ap->next = PrintAssln("\tpminsd\t%s,%s,%s\n", 
                                  GetDeref(op3),
                                  archvs4regs[-VIREGBEG-op1]);
         else
            ap->next = PrintAssln("\tpminsd\t%s,%s,%s\n", 
   ...
   case VIMIN_W8:
         if (op3 > 0)
            ap->next = PrintAssln("\tvpminsq\t%s,%s,%s\n", 
                                  GetDeref(op3),
                                  archvi8regs[-VIREGBEG-op2], 
                                  archvi8regs[-VIREGBEG-op1]);
         else
   ...
   #endif
   case VIMIN_W4:
            fko_error(__LINE__, "64 bit MIN not supported on YMM at all!");
         break;
   case VIMIN_W2:
            assert(op1 == op2);
         if (op3 > 0)
            ap->next = PrintAssln("\tpminsq\t%s,%s,%s\n", 
                                  GetDeref(op3),
                                  archvi2regs[-VIREGBEG-op1]);
         else
            ap->next = PrintAssln("\tpminsq\t%s,%s,%s\n", 
 ...
 */
   ...
   #endif
/*
 * NOTE: not implemented for AVX512.. so, no VSSHUF_W16
 */
   #if defined(AVX512) || defined(AVX2)  /* atleast AVX2 needed */
/*
 *    Implemented following options:
 *         AVX: 0, 7654FEDC, 765432BA/7654BABA, 76CD3289/7654BA99, B3A29180, 
 */
      case VSSHUF_W8:
         cp = imap2cmap(SToff[op3-1].i); /* return hex char */
 ...
 */
         else if(cp[7] == 11 && cp[6] == 10 && cp[5] == 9 && cp[4] == 8)    
         {
            ...
            {
               ap->next = PrintAssln("\tvperm2i128\t$0x20,%s,%s,%s\n",
                                     archvs8regs[-VIREGBEG-op2], /*src2==dest*/ 
                                     archvs8regs[-VIREGBEG-op1], 
                                     archvs8regs[-VIREGBEG-op1]);
            }
         }
         else if(cp[7] == 13 && cp[6] == 5 && cp[5] == 12 && cp[4] == 4)    
         {
            ...
            {
               ap->next = PrintAssln("\tvpunpckldq\t%s,%s,%s\n",
                                     archvs8regs[-VIREGBEG-op2], /* src2*/ 
                                     archvs8regs[-VIREGBEG-op1], 
                                     archvs8regs[-VIREGBEG-op1]);
            }
         }
         else
            fko_error(__LINE__, "Not implemented this vsshuf_w8 yet!\n"); 
         break;
   #endif
   case VSSHUF_W4:
            fko_error(__LINE__, "Not implemented VISHUF for SSE yet");
